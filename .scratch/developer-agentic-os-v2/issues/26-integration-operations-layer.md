# 26: Integration Operations Layer

**Triage:** ready-for-agent

**What to build:** Add a provider-neutral Integration Operations layer to the Agentic OS. GitHub and Vercel are the first actionable integrations. Sentry, Cloudflare, CodeRabbit, WorkOS, Clerk, Convex, NeonDB, Upstash, Email, and Slack begin as health, setup, or deferred integrations.
...
The user needs one operations surface that shows whether applications and development work are healthy, while keeping GitHub, Vercel, Sentry, Cloudflare, and the other services as systems of record. Problems should become actionable context inside the Agentic OS rather than isolated status cards.

## Solution

Build a provider-neutral Integration Operations layer scoped to the active Repository Context. GitHub and Vercel receive the first actionable MVP. The remaining daily tools begin as explicit health, setup, or deferred states and gain actions later without changing the dashboard contract.
...
- Keep provider operations separate from existing systems of record. The OS reads provider state and creates local Incoming Signals, Work Items, Skill Runs, or Artifacts when the user chooses.
...
- The supplied integration table is the roadmap input. GitHub and Vercel are the first actionable providers; all other listed tools are initially health-oriented or staged.
- Existing prior art includes the Integration Registry tests, API route contract tests, Incoming Signal Store tests, and repository-context isolation behavior.
- The local issue workflow is the source of truth because this repository has no configured remote.

## Completion

Implemented and verified through child tickets 27–31:

- Integration Health Foundation
- GitHub Operations MVP
- Vercel Operations MVP
- Integration Failure Triage
- Cross-Repository Operations Verification

Final validation passed: 60 unit/route tests, 32 browser tests, lint, and production build.
