...
- [ ] A scheduled workflow passes end to end in two isolated Repository Contexts.
- [ ] A provider-event workflow passes end to end through normalization, Incident grouping, diagnosis, approval, action, and audit inspection.
- [ ] Events, Incidents, policies, runs, Signals, actions, and outputs cannot cross repository boundaries.
- [ ] Reset clears only the selected repository's operational state.
- [ ] Existing Phase Three and Integration Operations unit, route, and browser behavior remains green.
- [ ] Full lint, typecheck, unit, E2E, and production-build validation passes.