...
export interface HostedObjectStore {
  put(content: string | Uint8Array, contentType: string): Promise<{ reference: string; contentType: string; size: number }>;
...
export class LocalHostedObjectStore implements HostedObjectStore {
  ...
  async get(reference: string): Promise<Uint8Array> {
    if (typeof reference !== "string" || reference.length !== 64 || !/^[a-f0-9]{64}$/.test(reference)) throw new Error("Invalid object reference.");
    const path = resolve(this.root, reference);
    const relativePath = relative(this.root, path);
    if (resolve(this.root, relativePath) !== path || relativePath !== reference || relativePath.includes("\0") || relativePath.includes("/") || relativePath.includes("\\")) throw new Error("Invalid object reference.");
