...
test("vercel adapter maps deployments and log links", async () => {
  ...
  const adapter = new VercelAdapter(projectEnv, async (input) => {
    ...
    return response({ deployments: [{ uid: "dpl_1", name: "example", url: "example-abc.vercel.app", state: "READY", created: 1893456000000, target: "production", meta: { githubCommitSha: "abc123" } }] });
  });
...
test("vercel adapter distinguishes configuration and provider failures", async () => {
  const missingProject = await new VercelAdapter({ VERCEL_TOKEN: "test-token" }, async () => response({})).getOperations();
  assert.equal(missingProject.status, "unconfigured");
  assert.match(missingProject.message, /VERCEL_PROJECT_ID/);

  const unauthorized = await new VercelAdapter(projectEnv, async () => response({}, 401)).getOperations();
  assert.equal(unauthorized.status, "unhealthy");
  assert.equal(unauthorized.failure?.kind, "authentication");

  const rateLimited = await new VercelAdapter(projectEnv, async () => response({}, 429)).getOperations();
  assert.equal(rateLimited.failure?.kind, "rate_limit");

  const timedOut = await new VercelAdapter(projectEnv, async () => { throw new Error("request timeout"); }).getOperations();
...
test("vercel adapter uses the active context project and reports failed deployments", async () => {
  ...
  try {
    ...
    await writeFile(join(root, ".vercel", "project.json"), JSON.stringify({ projectId: "prj_context", orgId: "team_context" }), "utf8");
    const result = await new VercelAdapter({ VERCEL_TOKEN: "test-token", VERCEL_PROJECT_ID: "wrong/global" }, async () => response({ deployments: [{ uid: "dpl_error", name: "broken", state: "ERROR", created: 1893456000000 }] }), root).getOperations();
