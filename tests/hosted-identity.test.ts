...
import { GET as listWorkspaces, POST as createWorkspace } from "../src/app/api/hosted/workspaces/route";
...
test("hosted workspaces are private, switchable, and audited per user", async () => {
  ...
  try {
    ...
    assert.deepEqual((await store.list("alice")).map((workspace) => workspace.name), ["Personal", "Client"]);
    ...
    assert.deepEqual((await store.audit("alice")).map((event) => event.action), ["workspace.created", "workspace.created", "workspace.selected"]);
  } finally {
...
test("hosted workspace JSON storage is rejected outside the explicit fixture boundary", async () => {
  ...
  try {
    ...
    try { await assert.rejects(() => new HostedWorkspaceStore(root).list("alice"), /fixture-only/); }
    finally { await rm(root, { recursive: true, force: true }); }
  } finally {
    if (previousNodeEnv === undefined) delete environment.NODE_ENV; else environment.NODE_ENV = previousNodeEnv;
    if (previousFixtureMode === undefined) delete environment.HOSTED_JSON_FIXTURE_MODE; else environment.HOSTED_JSON_FIXTURE_MODE = previousFixtureMode;
    if (previousAuthFixtureMode === undefined) delete environment.HOSTED_AUTH_FIXTURE_MODE; else environment.HOSTED_AUTH_FIXTURE_MODE = previousAuthFixtureMode;
  }
});
...
test("hosted routes require identity and isolate workspace selection", async () => {
  ...
  const created = await createWorkspace(request("route-alice", {
    method: "POST",
