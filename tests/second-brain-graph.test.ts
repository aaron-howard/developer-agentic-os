...
test("second brain graph contains repo, area, file, artifact, and skill nodes", async () => {
  ...
  try {
    ...
    const artifact = await new ArtifactStore(root).createArtifact({
      ...
      provenance: { repositoryId: repositoryId(root), repositoryRoot: root, workflowRefs: [{ kind: "skill", ref: "repo-summary" }] },
    });
...
test("second brain graph connects explicit signal, work item, handoff, artifact, skill, routine, and context relationships", async () => {
  ...
  try {
    ...
    const signal = await new IncomingSignalStore(root).create({ source: "manual", title: "Review migration", body: "Validate the graph", repositoryId: contextId });
    const workItem = await new WorkItemStore(root).create({ title: "Validate graph", repositoryId: contextId, contextRefs: [{ kind: "incoming_signal", ref: signal.id }] });
    const artifact = await new ArtifactStore(root).createArtifact({ name: "Migration evidence", type: "validation", content: "ok", contextRefs: [{ kind: "incoming_signal", ref: signal.id }, { kind: "work_item", ref: workItem.id }] });
    const handoffStore = new HandoffStore(root);
    const draft = await handoffStore.create({ id: contextId, name: "graph", path: root }, { title: "Graph handoff" });
    const finalized = await handoffStore.finalize(draft.id, contextId);
    const graph = await buildSecondBrainGraph(root);

    assert.ok(graph.nodes.some((node) => node.id === `repo_context:${contextId}` && node.type === "repo_context"));
    ...
    assert.ok(graph.links.some((link) => link.source === `incoming_signal:${signal.id}` && link.target === `repo_context:${contextId}` && link.type === "scoped_to"));
    assert.ok(graph.links.some((link) => link.source === `work_item:${workItem.id}` && link.target === `incoming_signal:${signal.id}` && link.type === "references"));
    assert.ok(graph.links.some((link) => link.source === `artifact:${artifact.id}` && link.target === `incoming_signal:${signal.id}` && link.type === "used_context"));
    assert.ok(graph.links.some((link) => link.source === `handoff:${draft.id}` && link.target === `work_item:${workItem.id}` && link.type === "includes"));
    assert.ok(graph.links.some((link) => link.source === `handoff:${draft.id}` && link.target === `artifact:${finalized.artifactId}` && link.type === "finalized_as"));
  } finally {
    await rm(root, { recursive: true, force: true });
  }
});
...
test("second brain graph supports clean first build and isolated multi-repository Local Store reset", async () => {
  ...
  try {
    ...
    const firstSignal = await new IncomingSignalStore(firstRoot).create({ source: "manual", title: "First repository", repositoryId: firstId });
    await new IncomingSignalStore(secondRoot).create({ source: "manual", title: "Second repository", repositoryId: secondId });
    ...
    assert.equal(resetGraph.nodes.some((node) => node.id === `incoming_signal:${firstSignal.id}`), false);
    assert.ok(secondGraph.nodes.some((node) => node.type === "incoming_signal" && node.metadata?.repositoryId === secondId));
  } finally {
